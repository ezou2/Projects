...
class BigInt {
private:
  std::vector<uint64_t> vector; //vector of bits
  bool negative_val; //if value is negative or not
...
private:
  ...
  BigInt abs() const;
  void fix_edge_cases();
  static std::vector<uint64_t> sum_magnitudes(const std::vector<uint64_t> &lhs, const std::vector<uint64_t> &rhs);
  static std::vector<uint64_t> diff_magnitudes(const std::vector<uint64_t> &lhs, const std::vector<uint64_t> &rhs);
  static std::vector<uint64_t> quotient_magnitudes(const std::vector<uint64_t> &lhs, const std::vector<uint64_t> &rhs);
  static int compare_magnitudes(const std::vector<uint64_t> &lhs, const std::vector<uint64_t> &rhs);
  static void carry_the_one(std::vector<uint64_t> &v, std::vector<uint64_t>::iterator i);
  static void borrow_the_one(std::vector<uint64_t> &v, std::vector<uint64_t>::iterator i);
  static void right_shift_by_one(std::vector<uint64_t> &v, std::vector<uint64_t>::iterator i);
  static BigInt divide(const BigInt &divisor, const BigInt &dividend, const BigInt &upper, const BigInt &lower);
  static void print_remainder(std::stringstream &s, const BigInt &dividend, const BigInt &divisor);
};
